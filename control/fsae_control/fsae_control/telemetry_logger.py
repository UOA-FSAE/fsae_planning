"""
Lightweight CSV telemetry logger for the control node (debug/tuning aid).

Writes two compact CSVs so planner problems can be separated from controller
problems offline. On close() the control CSV is rewritten with a scored,
commented header block (see "Score header" below).
...
`#`-commented dump of the launch-time configuration this run used — the
controller name and the launch-time flags that pick a path/speed source
(`map_path`, `path_map_path`, `v_max`, `v_min`, `stanley_gain`). The goal is
that a single CSV, on its own, is enough to reproduce the exact run it came
from.
"""
import csv
import math
...
def build_config_lines(controller: str, launch_flags: dict | None = None) -> list[str]:
    """
    ...
    ----------
    controller : str
        'stanley' -- whichever node built this.
    launch_flags : dict, optional
        Arbitrary key -> value pairs the CALLER already has from its own
        get_parameter() calls (map_path, path_map_path, v_max, v_min,
        stanley_gain, ...) -- this function doesn't know or care what a node
        declares, it just formats whatever dict it's handed. Empty/None
        values are logged as such (not skipped), since "map_path was empty"
        (-> live planner mode) is exactly the kind of thing worth being able
        to read back.
    """
...
class ControlLogger:
    def __init__(self, tag: str, log_dir: str = '', path_period: float = 1.0,
        ...
        # caller (stanley_controller.py's build_config_lines() call). Stored
        # here rather than written immediately because it's folded into the
        # SAME single-rewrite-at-close() mechanism the score header already
        # uses (see _write_score_header) -- one header write, not two.
    ...
    def set_config_lines(self, lines: list[str]) -> None:
        ...
        controller object is often constructed AFTER ControlLogger in a
        node's __init__; calling this any time before close() is fine, the
    ...
    def log_control(self, t, car_x, car_y, car_yaw, v_actual, v_desired,
        ...
        delta_cmd (rad) / a_cmd (m/s^2) are the raw steering/accel command
        pair; when omitted, delta_cmd falls back to steer_rad and a_cmd to
        0.0 so the Stanley controller (which has no longitudinal command)
        still logs and scores its lateral behaviour.
