...
Deliberately self-contained: the merge-and-deduplicate logic below started
as a copy of fsae_planning's planning/cone_map.py (ConeMap/_absorb). That
module has since been deleted from the live planner — centerline_planner.py
now plans from each boundary frame directly, with no cross-tick cone memory,
so a stale/misassociated cone can't linger in the live path — but THIS node's
job is exactly to accumulate the full lap for offline replay, so it keeps its
own copy of the merge logic rather than importing a module that no longer
exists. Not an import of fsae_planning either way — this package
(fsae_sim_perception) has no dependency on it beyond fsae_interfaces, and a
cross-ament_python-package import is more coupling than a ~20-line merge
routine is worth.
...
def _absorb(store: np.ndarray, obs: np.ndarray) -> np.ndarray:
    ...
    Originally copied from planning/cone_map.py's _absorb() — see module
    docstring for why this node keeps its own copy now that module is gone.
    """
    if len(obs) == 0:
        return store
    if len(store) == 0:
